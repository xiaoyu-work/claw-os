...
| Agent terminal frontend | Claw-owned ratatui presentation that consumes canonical conversation, task, model, tool and approval services directly; no external TUI process or compatibility protocol | `core/src/agent/terminal/`, `core/src/clawd/conversations.rs` |
...
| Memory and sessions | SQLite/FTS memory, semantic recall, session/message persistence, curation, checkpoints, and owner-scoped learned-memory reset that preserves execution evidence | `core/src/agent/memory/`, `core/src/session/`, `core/src/checkpoint.rs` |
...
| App development platform | Versioned, digest-pinned SDK/runtime/toolkit library artifact, independent of App releases and OS implementation checkouts | `packaging/app-platform.json`, `scripts/app_platform.py`, `docs/app-platform.md` |
...
override is not a local authorization shortcut. Registration checks the root
peer on the actual connection and preserves structured review refusals.
...
Backup and Recovery owns both `backup-center` and `system-snapshot`, with
...
Display control retains exact layout-file read authority and explicit
apply/restore confirmation; COSMIC output changes, kernel backlights and
owner-bound backup/restore state remain OS-owned.
...
Network observation, Wi-Fi, VPN and airplane permissions remain separate.
Optional Wi-Fi secret references require exact read authority; credential
loading, NetworkManager execution, profile validation and before/after state
collection remain OS-owned.
...
`gateway-webhook` completes the three planned Notification Delivery source
moves. Its JSON/raw POST, target flag/default lookup, independent HMAC signing
...
inline bytes in the owner-scoped Job. Worker protocol v15 introduced that
verified snapshot to `claw-agentd`; the runtime adds image blocks only to the
...
```text
cos agent chat
  -> in-process Claw ratatui renderer
  -> agent.conversation.* plus existing task/approval broker routes
  -> claw-agentd and the shared guarded Agent runtime above
```

The renderer is source-owned by Claw and never creates another Agent backend,
transport protocol, provider, tool registry, approval authority, audit log,
...
capabilities.
Generic Agent Web and native Desktop tool cards consume the same durable
progress stream but receive only tool identity, lifecycle status, latency and
returned-byte count. Successful result bodies remain inside the Agent
runtime/history trajectory; failed tools may include only the user-visible
projection's bounded, redacted error preview, which each presentation adapter
redacts and bounds again.
