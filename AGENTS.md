...
| Activity execution priority or shared queue admission | `core/src/activities/scheduling_policy.rs`, `core/src/agent/service.rs`, `docs/activities.md` | Owner-scoped SQLite CAS policy, authenticated broker and terminal controls, dynamic `Store::claim_one` ordering and bounded aging; never a Job field, authority grant, preemption or model decision |
...
| Agent terminal frontend or Codex TUI compatibility | `core/src/agent/terminal.rs`, `core/src/agent/tui_backend/`, `docs/agent-terminal.md` | Pinned upstream frontend, canonical `clawd/conversations.rs`, broker task/approval services, plain chat compatibility and packaging; never start a Codex Agent as fallback |
| Pinned terminal frontend sources or build artifacts | `terminal/MODULE.md`, `terminal/source.json` | Isolated upstream build, maintained patches, target-qualified artifact verification and attributions; building the frontend never selects a Codex Agent backend |
...
| App installation or requested-permission disclosure | `core/src/apps/installation.rs`, `core/src/apps/permission_review.rs` | Shared `apps/lint.rs`, CLI review/consent orchestration in `router/app_commands.rs`, verified snapshots, OS catalog, protected review controller; installation never grants runtime authority |
...
# OS Applet providers (independent workspace; no App source preparation)
...
Permission-related changes must also remain visible in the OS-generated
installation review, including conditional needs, argument constraints, AI
