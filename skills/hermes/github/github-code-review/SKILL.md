...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if GITHUB_TOKEN=$(cos kv get github_token 2>/dev/null) && [ -n "$GITHUB_TOKEN" ]; then
      :  # loaded from kernel KV store (verb: data.kv.read)
...
```bash
GITHUB_TOKEN=$(cos kv get github_token 2>/dev/null)
# Or run the inline setup block from the top of this skill (covers gh auth too)
```
