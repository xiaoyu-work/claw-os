"""Comprehensive tests for the aos CLI tool.

Invokes aos as a subprocess (like a real caller would) and validates JSON output.
...
AOS = os.path.abspath(AOS)


def run_aos(*args, stdin_data=None, cwd=None):
    """Run the aos CLI and return (parsed_json, returncode)."""
    result = subprocess.run(
        [sys.executable, AOS, *args],
        capture_output=True,
        text=True,
        input=stdin_data,
        cwd=cwd,
...
class TestFsLs:
    def test_ls_current_dir(self, tmp_path):
        ...
        data, rc = run_aos("fs", "ls", str(tmp_path))
        assert rc == 0
    ...
    def test_ls_default_cwd(self, tmp_path):
        ...
        data, rc = run_aos("fs", "ls", cwd=str(tmp_path))
        assert rc == 0
        assert "hello" in data["files"]

    def test_ls_nonexistent(self, tmp_path):
        data, rc = run_aos("fs", "ls", str(tmp_path / "nope"))
        assert rc != 0
        assert "error" in data
...
class TestFsPwd:
    def test_pwd(self, tmp_path):
        data, rc = run_aos("fs", "pwd", cwd=str(tmp_path))
        assert rc == 0
...
class TestFsRead:
    ...
    def test_read_missing_file(self, tmp_path):
        data, rc = run_aos("fs", "read", str(tmp_path / "nope.txt"))
        assert rc != 0
        assert "error" in data

    def test_read_no_args(self):
        data, rc = run_aos("fs", "read")
        assert rc != 0
        assert "error" in data
...
class TestFsWrite:
    def test_write_file(self, tmp_path):
        ...
        data, rc = run_aos("fs", "write", str(target), stdin_data="hello")
        assert rc == 0
        assert data["bytes"] == 5
    ...
    def test_write_creates_parent_dirs(self, tmp_path):
        ...
        data, rc = run_aos("fs", "write", str(target), stdin_data="nested")
        assert rc == 0
        assert target.read_text() == "nested"

    def test_write_empty(self, tmp_path):
        target = tmp_path / "empty.txt"
        data, rc = run_aos("fs", "write", str(target), stdin_data="")
        assert rc == 0
        assert data["bytes"] == 0
        assert target.read_text() == ""

    def test_write_no_args(self):
        data, rc = run_aos("fs", "write", stdin_data="x")
        assert rc != 0
        assert "error" in data
    ...
    def test_write_overwrites(self, tmp_path):
        ...
        data, rc = run_aos("fs", "write", str(target), stdin_data="new")
        assert rc == 0
...
class TestFsRm:
    ...
    def test_rm_directory(self, tmp_path):
        ...
        data, rc = run_aos("fs", "rm", str(d))
        assert rc == 0
        assert not d.exists()

    def test_rm_missing(self, tmp_path):
        data, rc = run_aos("fs", "rm", str(tmp_path / "nope"))
        assert rc != 0
        assert "error" in data

    def test_rm_no_args(self):
        data, rc = run_aos("fs", "rm")
        assert rc != 0
        assert "missing argument" in data["error"]
...
class TestFsMkdir:
    def test_mkdir(self, tmp_path):
        ...
        data, rc = run_aos("fs", "mkdir", str(target))
        assert rc == 0
    ...
    def test_mkdir_existing(self, tmp_path):
        ...
        data, rc = run_aos("fs", "mkdir", str(tmp_path))
        assert rc == 0

    def test_mkdir_no_args(self):
        data, rc = run_aos("fs", "mkdir")
        assert rc != 0
        assert "missing argument" in data["error"]
...
class TestExecRun:
    def test_run_echo(self):
        data, rc = run_aos("exec", "run", "echo", "hello")
        assert rc == 0
    ...
    def test_run_exit_code(self):
        data, rc = run_aos("exec", "run", "false")
        assert rc == 0  # aos itself exits 0; the inner command's code is in JSON
        assert data["exit_code"] != 0

    def test_run_stderr(self):
        data, rc = run_aos("exec", "run", "sh", "-c", "echo err >&2")
        assert rc == 0
        assert "err" in data["stderr"]

    def test_run_not_found(self):
        data, rc = run_aos("exec", "run", "nonexistent_command_xyz_12345")
        assert rc != 0
        assert "error" in data
        assert "not found" in data["error"]

    def test_run_no_args(self):
        data, rc = run_aos("exec", "run")
        assert rc != 0
        assert "error" in data
...
class TestNetFetch:
    ...
    Since aos runs as a subprocess, we can't mock its imports directly.
    Instead we create a wrapper script that patches urllib before running aos.
    """

    def _run_fetch_with_mock(self, tmp_path, url, mock_status=200, mock_body="ok",
                              raise_http_error=False, raise_generic=False):
        """Create a wrapper that patches urllib and runs the aos fetch logic."""
        ...
        wrapper.write_text(textwrap.dedent(f"""\
            ...
            # Add aos to importable path by running its main
            sys.argv = ["aos", "net", "fetch", "{url}"]

            aos_path = {AOS!r}

            # We need to mock urllib.request.urlopen before aos uses it
            ...
            with unittest.mock.patch("urllib.request.urlopen", side_effect=fake_urlopen):
                ...
                with open(aos_path) as f:
                    code = f.read()
                exec(compile(code, aos_path, "exec"), {{"__name__": "__main__"}})
        """))
        result = subprocess.run(
...
class TestSysEnv:
    def test_env_returns_dict(self):
        data, rc = run_aos("sys", "env")
        assert rc == 0
        assert isinstance(data["env"], dict)

    def test_env_contains_path(self):
        data, _ = run_aos("sys", "env")
        assert "PATH" in data["env"]

    def test_env_custom_var(self):
        """Verify a custom env var is visible."""
        result = subprocess.run(
            [sys.executable, AOS, "sys", "env"],
            capture_output=True,
            text=True,
            env={**os.environ, "AOS_TEST_VAR": "12345"},
            timeout=15,
        )
        data = json.loads(result.stdout)
        assert data["env"]["AOS_TEST_VAR"] == "12345"
