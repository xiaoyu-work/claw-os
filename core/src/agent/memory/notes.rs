...
pub struct NoteSearch {
    ...
    /// Files consulted, including `partially_searched_name` when a limit stops the scan.
    pub searched_names: Vec<String>,
    pub partially_searched_name: Option<String>,
...
impl NotesStore {
        ...
        F: FnOnce(Option<String>) -> String,
    {
        let p = self.path_of(name)?;
        self.ensure_dir()?;
        filelock::update_locked::<_, std::convert::Infallible>(&p, |current| Ok(transform(current)))
        ...
        for name in names {
            ...
            for matched in pattern.find_iter(&text) {
                if result.hits.len() == limit {
                    ...
                    result.partially_searched_name = Some(name);
                    return Ok(result);
                }
