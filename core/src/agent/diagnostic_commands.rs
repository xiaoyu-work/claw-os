...
pub(super) fn usage_public_cmd(args: &[String]) -> Result<Value, String> {
    if args.is_empty()
        || args
            .first()
...
pub(crate) fn usage_for_current_context(args: &[String]) -> Result<Value, String> {
    ...
    #[cfg(all(unix, not(test)))]
    {
        if crate::paths::current_owner_uid_override().is_some() {
            ...
            crate::clawd::agent_client::usage(args)
        }
    }
}
...
fn usage_from_parsed_reader(
    ...
    let summary =
        crate::agent::llm::usage::aggregate_reader_filtered(reader, &parsed.query, max_bytes)?;
...
fn parse_usage_query(args: &[String]) -> Result<ParsedUsageQuery, String> {
    ...
    let scope_omitted = args.first().is_none_or(|scope| scope.starts_with("--"));
    let scope = args
        .first()
        .cloned()
