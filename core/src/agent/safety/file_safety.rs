...
///
/// The classifier is **purely lexical** — it does not stat the path or
/// resolve symlinks. The caller is expected to canonicalise first if
/// they want to defeat `..` traversal tricks.
pub fn classify(path: &Path) -> FileSafety {
    ...
    // 4. System directories — Unix prefix match.
    for prefix in UNIX_SYSTEM_PREFIXES {
        // Exact match or prefix followed by '/' so /etc matches
        // /etc/passwd but not /etcd-data.
...
/// matching. Lower-cases on Windows (where paths are case-insensitive)
/// to make `/.SSH/` and `/.ssh/` collide.
