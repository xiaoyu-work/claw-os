...
impl OpenAICompatProvider {
    ...
    async fn request_target(
        ...
        if alias_is_copilot(&self.cfg.alias) {
            ...
            let wire_api =
                super::copilot_auth::wire_api_for_model(&token, &self.cfg.model)
                    .await
                    .map_err(map_copilot_error)?;
...
fn map_copilot_error(error: super::copilot_auth::CopilotAuthError) -> LlmError {
    ...
    match error {
        ...
        CopilotAuthError::Http { status: 401 | 403, .. } => LlmError::Auth,
        CopilotAuthError::Http { status, body } => LlmError::Provider {
            status,
            message: crate::agent::llm::redact_body_for_error(&body),
        },
        CopilotAuthError::Network(message) => LlmError::NotConfigured(format!(
            "copilot model discovery failed: {message}"
        )),
...
impl Provider for OpenAICompatProvider {
    ...
    async fn chat_stream(
        ...
        match target.wire_api {
            ...
            super::copilot_auth::CopilotWireApi::Responses => Ok(
                responses_wire::ResponsesStream::new(
                    bytes_stream,
                    model,
                    self.cfg.pool.clone(),
                    lease,
...
/// OpenAI Responses wire adapter used by Copilot models whose live
/// `/models` entry advertises `/responses` but not `/chat/completions`.
pub(crate) mod responses_wire {
    ...
    fn message_to_input_items(message: &crate::agent::llm::Message) -> Vec<serde_json::Value> {
        ...
        let flush_content =
            |items: &mut Vec<serde_json::Value>, content: &mut Vec<serde_json::Value>| {
                if !content.is_empty() {
            ...
            match block {
                ...
                ContentBlock::Reasoning {
                    ...
                    if let Some(encrypted_content) =
                        encrypted_content.as_deref().filter(|_| id.starts_with("rs"))
                    {
                        items.push(serde_json::json!({
                            "type": "reasoning",
                            "id": id,
    ...
    fn response_error(error: &ResponseError) -> LlmError {
            ...
            .as_str()
        {
            ...
            "authentication_error" | "invalid_api_key" | "unauthorized" | "forbidden" => {
                LlmError::Auth
            }
        ...
        pub(crate) fn process(
            ...
            match event {
                ...
                ResponsesEvent::Completed { response } => {
                    self.finish_from_response(response, None)
                }
    ...
    impl ResponsesStream {
        ...
        fn report_failure_once(
            &mut self,
            class: crate::agent::llm::credential_pool::FailureClass,
        ) {
            if self.accounted {
                return;
            }
...
mod tests {
    ...
    fn builds_responses_body_with_tool_history() {
        ...
        let body =
            responses_wire::build_request_body(&request, "gpt-5.6-sol", true);
    ...
    fn responses_does_not_replay_reasoning_without_encrypted_state() {
        ...
        let body =
            responses_wire::build_request_body(&request, "gpt-5.6-sol", false);
        assert!(
            body["input"]
                .as_array()
                .unwrap()
    ...
    fn responses_stream_emits_text_tool_and_terminal_usage() {
        ...
        let mut converter =
            responses_wire::ResponsesStreamConverter::new("gpt-5.6-sol".into());
    ...
    async fn responses_stream_rejects_eof_without_terminal_event() {
        ...
        let stream = responses_wire::ResponsesStream::new(
            bytes,
            "gpt-5.6-sol".into(),
            None,
            None,
