...
fn dispatch_wizard(modality: Modality, verify_after: bool) -> Result<Value, String> {
    match modality {
        ...
        Modality::Tts => media::wizard_media(media::tts_spec(), verify_after),
        Modality::Stt => media::wizard_media(media::stt_spec(), verify_after),
        Modality::ImageGen => media::wizard_media(media::imagegen_spec(), verify_after),
        Modality::Embed => media::wizard_media(media::embed_spec(), verify_after),
    ...
    match modality {
        ...
        Modality::All => {
            ...
            for m in [
                ...
                report.insert(m.name().into(), media::verify_media(m));
            }
            Ok(json!({"verified": report}))
        }
        other => Ok(media::verify_media(other)),
    }
}
    ...
    match modality {
        ...
        Modality::All => {
            ...
            for m in [
                ...
                map.insert(m.name().into(), media::status_media(m));
            }
            map.insert(
                "config_path".into(),
        ...
        other => Ok(media::status_media(other)),
    }
}
...
pub fn status_for(modality: Modality) -> Value {
    match modality {
        ...
        other => media::status_media(other),
    }
}
...
fn providers_media(modality: Modality) -> Value {
    ...
    let default_provider = media::default_provider_for(spec);
...
fn apply_media(spec: &'static media::ModalitySpec, args: ApplyArgs) -> Result<Value, String> {
    ...
    if media::is_embed_spec(spec) && media::is_local_embed_provider(provider.name) {
        if let Err(err) = media::local_embed_precheck(&json!({}), Some(&model)) {
            return Err(json!({
                "error": "local embedding stack unavailable",
                "details": media::local_embed_unavailable_details(&err),
