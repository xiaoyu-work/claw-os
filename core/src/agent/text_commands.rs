...
/// profile and due reminders; runtime adds explicit surface context and
...
pub(super) fn prompt_cmd(args: &[String]) -> Result<Value, String> {
    ...
    let (prompt, candidate_segments) =
        crate::agent::prompt::build_system_prompt_traced(extra_ref, None);
    let prelude_segments: Vec<_> = candidate_segments
        .into_iter()
        .filter(|segment| !segment.class().is_policy())
        .collect();
    ...
    let mut builder = crate::agent::context::packet::ContextBuilder::new(
        ...
        budget.input_tokens.saturating_sub(
            crate::agent::context::compressor::estimate_text_tokens(&prompt)
                .saturating_add(prelude_segments.iter().fold(0u32, |total, segment| {
                    total.saturating_add(
                        crate::agent::context::compressor::estimate_text_tokens(&segment.content)
                            .saturating_add(4),
                    )
                }))
                .saturating_add(16),
