...
pub fn enforce_worker_binary(binary: &Path) -> Result<(), Refusal> {
    enforce_component_binary("claw-agentd", binary)
}
...
pub fn enforce_worker_binary_in(store: &FloorStore, binary: &Path) -> Result<(), Refusal> {
    enforce_component_binary_in(store, "claw-agentd", binary)
}

pub fn enforce_component_binary(component: &str, binary: &Path) -> Result<(), Refusal> {
    enforce_component_binary_in(&FloorStore::system(), component, binary)
}

pub fn enforce_component_binary_in(
    store: &FloorStore,
    component: &str,
    binary: &Path,
) -> Result<(), Refusal> {
    ...
    let Some(recorded) = floor.components.get(component) else {
        return Ok(());
    };
    // Only installed components are measured. A development tree points
    // an override somewhere else entirely, and the floor has
    ...
    if installed.as_os_str().is_empty() || binary != installed {
        return Ok(());
    }
    let measured = super::floor::measure_component(component, binary).map_err(|error| {
        refuse(
            super::decide::class::FLOOR_UNAVAILABLE,
            format!("cannot verify the {component} binary: {error}"),
        )
    })?;
    if measured.sha256 != recorded.sha256 {
        return Err(refuse(
            super::decide::class::ARTIFACT_MISMATCH,
            format!(
                "the installed {component} binary does not match the content recorded for this \
                 release; refusing to spawn a replaced component"
            ),
        ));
    }
    Ok(())
