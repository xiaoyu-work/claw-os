...
impl ObjectResolver {
    pub(crate) fn arguments<'a>(&self, manifest: &'a Manifest) -> Result<&'a [Arg], String> {
        if let Some(operation) = manifest.operations.get(&self.operation) {
            if operation.stdin {
                return Err("resolver operations cannot require caller stdin".to_string());
            }
            return Ok(&operation.args);
        }
        manifest
            .mcp_tool_for_command(&self.operation)
            .map(|tool| tool.args.as_slice())
    }
}
...
pub(super) fn validate(manifest: &Manifest) -> Result<(), ManifestError> {
    ...
    for (name, object) in &manifest.objects {
        ...
        for arg in args {
            if arg.required_when.is_some() {
                return Err(invalid(
                    name,
                    "resolver arguments cannot depend on conditional inputs",
                ));
            }
