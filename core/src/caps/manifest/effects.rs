...
pub(super) fn validate(manifest: &Manifest) -> Result<(), ManifestError> {
    let declarations = manifest
        .operations
        .iter()
        .map(|(name, operation)| (name, &operation.args, &operation.effects))
        .chain(manifest.mcp.iter().flat_map(|service| {
            service
                .tools
                .iter()
                .map(|tool| (&tool.name, &tool.args, &tool.effects))
        }));
    for (name, args, effects) in declarations {
        ...
        for effect in effects {
            ...
            if let Some(target) = &effect.target_arg {
                let arg = args
                    ...
                    .ok_or_else(|| invalid("target_arg must name a declared argument"))?;
