    ...
    fn open(mounts: &[super::policy::Mount]) -> Result<Self, String> {
        ...
        for mount in mounts {
            ...
            if before.file_type().is_symlink() {
                return Err(format!(
                    "worker mount source became a symlink: {}",
...
fn inside_extension_host() -> bool {
    let uid = unsafe { libc::geteuid() as u32 };
    let end = crate::extension_host::identity::FIRST_UID
        + crate::extension_host::identity::IDENTITY_COUNT
        - 1;
    (crate::extension_host::identity::FIRST_UID..=end).contains(&uid)
        && std::env::var("COS_EXTENSION_CHILD_ISOLATION").as_deref() == Ok("1")
...
fn apply_rlimits(limits: &super::policy::Limits, nproc_ceiling: u64) -> std::io::Result<()> {
    let set = |resource: libc::__rlimit_resource_t, value: u64| -> std::io::Result<()> {
        ...
        let value = (value as libc::rlim_t).min(inherited.rlim_max);
