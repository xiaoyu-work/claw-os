...
fn authorize_session(session_id: &str, peer_pid: u32, requested: &[Cap]) -> Result<(), String> {
    let session = crate::proc::session_info_by_id(session_id)
        .ok_or_else(|| format!("printer-manager session not found: {session_id}"))?;
    if session.app_id.as_deref() != Some("printer-manager") {
        return Err("printing is restricted to the printer-manager App".to_string());
    }
    if session.pending_bind || session.pid == 0 {
        return Err("printer-manager session is not bound to a process".to_string());
    }
    let expected_start = session
        .start_time_ticks
        .ok_or_else(|| "printer-manager session has no process identity".to_string())?;
    if crate::proc::read_start_time_ticks_pub(session.pid) != Some(expected_start) {
        return Err("printer-manager session process identity is stale".to_string());
    }
    if !crate::proc::process_descends_from(peer_pid, session.pid) {
        return Err("printer request did not originate from the authorized session".to_string());
    }
