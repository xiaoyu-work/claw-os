...
fn requires_root_peer(command: super::routes::Command) -> bool {
    command == super::routes::Command::AppSessionRegister
        || matches!(command, super::routes::Command::AppGuiLaunch
            | super::routes::Command::AppGuiWait | super::routes::Command::AppGuiStop)
...
pub fn request_blocking(
    ...
    if requires_root_peer(request.command) {
        ...
        stream.set_write_timeout(timeout).map_err(|error| {
            ClientError::before_dispatch(format!("set review write deadline: {error}"))
        })?;
    }
    let body =
...
fn require_root_broker_peer(stream: &std::os::unix::net::UnixStream) -> Result<(), ClientError> {
    ...
    let expected_length = std::mem::size_of::<libc::ucred>() as libc::socklen_t;
