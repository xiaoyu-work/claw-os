//! Owner-scoped Activity pending-admission priority. It never supplies
//! capability, consent, budget, completion, or execution authority.
...
pub fn get(params: Value, client: &ClientIdentity) -> Result<Value, BrokerError> {
    ...
    let request: body::ActivitySchedulingPolicyGet = decode(params)?;
    ...
    let policy = service
        .scheduling_policy(owner, &activity.id)
        .map_err(service_error)?;
    Ok(json!({
        "schema": 1,
...
pub fn set(params: Value, client: &ClientIdentity) -> Result<Value, BrokerError> {
    let owner = owner(client)?;
    let request: body::ActivitySchedulingPolicySet = decode(params)?;
    activities::validate_id(request.id.as_str()).map_err(service_error)?;
    let service = activities::open_default().map_err(service_error)?;
    encode(
        service
...
mod tests {
    include!(concat!(
        ...
        "/test/unit/clawd/activity_scheduling_policy.rs"
    ));
}
