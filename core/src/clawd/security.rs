...
fn authorize_session(session_id: &str, peer_pid: u32) -> Result<(), String> {
    let session = crate::proc::session_info_by_id(session_id)
        .ok_or_else(|| format!("security-center session not found: {session_id}"))?;
    if session.app_id.as_deref() != Some("security-center") {
        return Err("security analysis is restricted to the security-center App".to_string());
    }
    if session.pending_bind || session.pid == 0 {
        return Err("security-center session is not bound to a process".to_string());
    }
    let expected_start = session
        .start_time_ticks
        .ok_or_else(|| "security-center session has no process identity".to_string())?;
    if crate::proc::read_start_time_ticks_pub(session.pid) != Some(expected_start) {
        return Err("security-center session process identity is stale".to_string());
    }
    if !crate::proc::process_descends_from(peer_pid, session.pid) {
        return Err("security request did not originate from the authorized session".to_string());
    }
    ...
    let requested = Cap::new(Verb::SYS_SECURITY, Scope::name("audit"));
    if !caps.covers(&requested) {
        return Err("security-center session lacks sys.security:audit".to_string());
    }
    Ok(())
}
