...
fn service_help() -> Value {
    json!({
        "backend": "clawd",
        "subcommands": [
            "submit  \"<prompt>\" [--session ID] [--after-task ID] [--activity ID] [--workspace PATH] [--max-turns N] [--model ID]",
...
fn service_submit_params(args: &[String]) -> Result<Value, String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--model" => {
                let value = args.get(i + 1).ok_or("--model needs a value")?;
                crate::agent::service::validate_requested_model(Some(value))?;
    ...
    let prompt = prompt
        ...
        .ok_or("usage: cos agent service submit \"<prompt>\" [--session ID] [--after-task ID] [--activity ID] [--workspace PATH] [--max-turns N] [--model ID]")?;
    let mut params = json!({ "prompt": prompt });
    if let Some(session_id) = session_id {
        params["session_id"] = json!(session_id);
    }
...
mod tests {
    include!(concat!(
        ...
        "/test/unit/clawd/agent_client.rs"
    ));
}
