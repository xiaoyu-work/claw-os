...
pub(crate) fn render_mcp_result(
    result: &crate::agent::tools::mcp::protocol::CallToolResult,
) -> (String, bool) {
    ...
    let chunks: Vec<_> = result
        ...
        .map(|item| match item {
            ...
            ContentItem::Image { mime_type, .. } => {
                format!("[image content omitted ({mime_type})]")
            }
        })
        .collect();
    ...
    (body, result.is_error.unwrap_or(false))
}
