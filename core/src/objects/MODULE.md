...
Its `operation` names the ordinary public App command: an explicit operation
takes precedence, otherwise the existing exact `<app-id>.<command>` MCP tool
is selected. MCP-only Apps need no synthetic legacy entrypoint. Both metadata
validation and execution use that same selection contract.
