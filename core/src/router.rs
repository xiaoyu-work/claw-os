    ...
    if name == "__location" {
        ...
        while index < args.len() {
            ...
            match args[index].as_str() {
                ...
                "--accuracy" => {
                    return Err("duplicate internal location --accuracy".to_string())
                }
...
fn builtin_apps() -> Vec<(
    ...
    vec![
        ...
        ("agent", "OS-native agent subsystem — clawd-backed runtime, memory, skills, LLM providers, tools, and tasks", vec![
            ...
            ("ask", "Single-shot prompt with full tool/memory loop: cos agent ask \"<prompt>\" [--full]. Default prints just the model's plain-text answer; add --full to get the JSON envelope with provider/model/session/task ids."),
