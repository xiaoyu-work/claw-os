...
fn builtin_apps() -> Vec<(
    ...
    vec![
        ...
        ("agent", "OS-native agent subsystem — runtime, memory, skills, LLM providers, tools (Phase 0 skeleton)", vec![
            ...
            ("curator", "Distil a recorded session into a draft skill manifest: cos agent curator propose <session_id> [--accept] [--limit <n>] [--no-require-acceptance] [--min-tools <n>] [--min-turns <n>] [--no-save] | drafts list [--status proposed|accepted|rejected] | drafts show <id> | drafts accept <id> [--note ...] | drafts reject <id> [--note ...] | drafts delete <id> | drafts retitle <id> <new title> | drafts auto-title <id> [--seed description|title|both] [--dry-run]"),
