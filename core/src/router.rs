...
fn builtin_apps() -> Vec<(
    ...
    vec![
        ...
        ("agent", "OS-native agent subsystem — runtime, memory, skills, LLM providers, tools (Phase 0 skeleton)", vec![
            ...
            ("mcp", "MCP (Model Context Protocol) bridge — server exposes the cos agent tool catalogue (`status|serve [--allow <tool>] [--deny <tool>]`); client probes/invokes a remote MCP subprocess (`probe --cmd <bin> [--arg ...]|call --cmd <bin> [--arg ...] <tool> [--input <json>]`)"),
            ...
            ("curator", "Distil a recorded session into a draft skill manifest, then optionally have an LLM author the SKILL.md body: cos agent curator propose <session_id> [--accept] [--limit <n>] [--no-require-acceptance] [--min-tools <n>] [--min-turns <n>] [--no-save] | scan [--limit N] [--save] [--reprocess] [--message-limit N] [--min-tools N] [--min-turns N] [--no-require-acceptance] | drafts list [--status proposed|accepted|rejected] | drafts show <id> | drafts accept <id> [--note ...] | drafts reject <id> [--note ...] | drafts delete <id> | drafts retitle <id> <new title> | drafts auto-title <id> [--seed description|title|both] [--dry-run] | author <draft_id> [--model <name>] [--primary] [--write] [--out <path>] [--force] [--limit <n>]"),
            ...
            ("vision", "Preview how an image attachment would be routed (native vision call vs local OCR vs skip) given a policy: cos agent vision route --bytes N --mime <m> [--intent <general|extract-text|identify|caption>] [--provider-vision] [--vision-disabled] [--max-native-bytes N] [--ocr-available] | cos agent vision route --file <path> [...] | cos agent vision sniff --file <path> | --url <url> [--head-bytes N] | cos agent vision analyze --file <p> | --url <u> | --base64 <d> --mime <m> --prompt <text> [--system <text>] [--max-tokens N] [--provider <name>] [--model <name>] [--fetch-timeout-ms N]"),
