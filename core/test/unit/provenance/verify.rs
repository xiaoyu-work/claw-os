...
#[cfg(unix)]
#[test]
fn owner_trust_receipt_allows_independent_reverification_without_owner_files() {
    let root = tmpdir("owner-receipt");
    let (package_dir, key) = signed_app(&root, "receipt-app", "print('ok')\n");
    let trust_root = root.join("trust");
    let trust = trust_for(&[&key], &trust_root);
    let options = VerifyOptions::new(PackageKind::App).expect_id("receipt-app");
    let verified = verify_package(&package_dir, &options, &trust).unwrap();
    assert_eq!(verified.tier(), TrustTier::User);
    let receipt = verified.verification_receipt().unwrap();

    fs::remove_dir_all(&trust_root).unwrap();
    let reverified = verify_package_with_receipt(&package_dir, &options, &receipt).unwrap();
    assert_eq!(reverified.content_digest(), verified.content_digest());
    assert_eq!(reverified.tier(), TrustTier::User);

    let mut substituted = receipt;
    let PackageReceiptTrust::Publisher {
        public_key_sha256, ..
    } = &mut substituted.trust
    else {
        panic!("expected publisher receipt");
    };
    *public_key_sha256 = "0".repeat(64);
    assert!(verify_package_with_receipt(&package_dir, &options, &substituted).is_err());
}
