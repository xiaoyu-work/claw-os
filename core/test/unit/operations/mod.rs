...
#[test]
fn omitted_optional_target_remains_explicitly_unresolved() {
    let _lock = crate::test_env::lock_env();
    let root = tempfile::tempdir().unwrap();
    ...
    std::fs::write(dir.join("app.json"), json!({
        ...
        "operations":{"list":{"label":{"en":"List"},"args":[
            {"name":"provider","kind":"name","binding":"flag","choices":["local","google","outlook"]}
    ...
    let selected = preview(&app, "list", &["--provider".into(), "local".into()]).unwrap();
    assert!(selected.unresolved_arguments.is_empty());
    assert_eq!(selected.effects[0].requested_targets, ["local"]);
}

#[test]
fn mcp_only_command_previews_its_own_effects_without_an_operation_entry() {
    let _lock = crate::test_env::lock_env();
    let root = tempfile::tempdir().unwrap();
    let dir = root.path().join("demo");
    std::fs::create_dir(&dir).unwrap();
    std::fs::write(
        ...
        json!({
            ...
            "mcp":{"entry":"server.py","tools":[{
                "name":"demo.get","summary":{"en":"Read through MCP"},
                "args":[{"name":"key","kind":"name","required":true}],
                "effects":[{
                    "kind":"read","label":{"en":"Read the requested key"},
                    "target_arg":"key","recovery":"not_applicable"
                }]
            }]}
        })
        .to_string(),
    ...
    let app = crate::apps::find_verified(root.path(), "demo").unwrap();
    assert!(app.manifest.operations.is_empty());
    assert!(!dir.join("main.py").exists());
    let preview = preview(&app, "get", &["release.status".into()]).unwrap();
    assert_eq!(preview.operation_label, "Read through MCP");
    assert_eq!(preview.effects[0].requested_targets, ["release.status"]);
    assert_eq!(preview.effects[0].kind, EffectKind::Read);
    assert!(!preview.authorization_checked && !preview.executed && !preview.effects_confirmed);
}
