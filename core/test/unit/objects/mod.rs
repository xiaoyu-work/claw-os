...
#[test]
fn mcp_only_objects_are_described_from_the_verified_manifest_without_execution() {
    let _lock = crate::test_env::lock_env();
    let root = tempfile::tempdir().unwrap();
    let app = root.path().join("demo");
    std::fs::create_dir(&app).unwrap();
    ...
    std::fs::write(
        app.join("server.py"),
        "raise AssertionError('metadata executed MCP')\n",
    )
    .unwrap();
    crate::test_env::sign_test_package(&app, crate::provenance::PackageKind::App, "demo");
    let catalog = InstalledObjectCatalog::new(root.path());
    let (retained, description) = catalog.prepare(&object("entry", "--schema", None)).unwrap();
    assert!(retained.manifest.operations.is_empty());
    assert_eq!(description.invocation.operation, "get");
    assert_eq!(description.invocation.args, ["--", "--schema"]);
    assert!(crate::apps::command_uses_mcp_cli(
        &retained.manifest,
        &description.invocation.operation
    ));
    assert_eq!(
        retained.manifest.mcp_tool_for_command("get").unwrap().name,
        "demo.get"
    );
    assert!(!app.join("main.py").exists());
}
