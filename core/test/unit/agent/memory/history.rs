...
#[test]
fn invalid_page_windows_never_return_an_unbounded_or_misleading_source() {
    assert!(text_page("source", 0, 0).is_err());
    assert!(text_page("source", 0, MAX_READ_CHARS + 1).is_err());
    assert!(text_page("source", 7, 1).is_err());
    assert!(text_page("source", usize::MAX, 1).is_err());
    let end = text_page("source", 6, 1).unwrap();
    assert!(end.content.is_empty());
    assert!(!end.source_complete);
    assert!(end.next_offset.is_none());
    assert!(text_page("", 0, 1).unwrap().source_complete);
}
