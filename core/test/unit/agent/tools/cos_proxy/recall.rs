...
async fn recall_search_and_show_do_not_expose_reverted_rows() {
    ...
    let search = exec(
        &t,
        json!({"command": "search", "query": "REVERTED_SENTINEL"}),
    )
    .await;
    assert!(!search.is_error, "{}", search.content);
    assert!(recall_json(&search)["hits"].as_array().unwrap().is_empty());

    let show = exec(&t, json!({"command": "show", "message_id": id})).await;
    assert!(show.is_error);
    assert!(!show.content.contains("REVERTED_SENTINEL"));
}
