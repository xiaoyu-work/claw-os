...
fn activity_projection_filters_owner_and_association_before_limit_across_buckets() {
    ...
    for (index, status) in statuses.into_iter().enumerate() {
        let mut job = Job::new_pending("wanted".into(), None, None, None, None, Some(1001), None);
        ...
        for index in 0..24 {
            let mut other =
                Job::new_pending("other".into(), None, None, None, None, Some(1001), None);
            other.status = status;
...
async fn activity_context_is_recorded_as_untrusted_transient_not_prompt_or_user_history() {
    ...
    let user_blocks = captured
        ...
        .collect::<Vec<_>>();
    let envelope = user_blocks
        .iter()
        .filter_map(|text| crate::agent::trust::envelope::parse(text))
        .find(|envelope| envelope.payload.contains(&activity.goal))
        .expect("Activity planning data remains in a labelled user envelope");
    assert_eq!(
    ...
    assert_eq!(
        envelope.class,
        crate::agent::trust::TrustClass::UntrustedExternalContent
    );
    let user_text = user_blocks.join("\n");
    ...
    assert!(rows.iter().any(|row| {
        row.role == "injected"
            && crate::agent::trust::envelope::parse(&row.content).is_some_and(|envelope| {
                envelope.source.kind() == crate::agent::trust::SourceKind::TransientAppContext
                    && envelope.payload.contains(&activity.goal)
            })
    }));
}
...
fn task_can_explicitly_disable_conversation_memory() {
    ...
    let job = store.publish(job).unwrap();
