...
fn an_activity_asks_for_held_caps_once_and_cannot_borrow_a_broader_approval() {
    ...
    binding.check_delegated(std::slice::from_ref(&cap)).unwrap();
    assert!(
        !crate::approvals::has_approved_exact_grant_for_owner("cli-test", &cap, Some(4242))
            .unwrap()
    );
    assert!(
        crate::approvals::has_approved_exact_grant_for_owner("cli-test", &broad, Some(4242))
            .unwrap()
    );
...
fn forced_and_missing_app_capabilities_are_settled_all_or_none() {
    ...
    assert_eq!(approval_requests(&error).len(), 1);
    assert!(
        crate::approvals::has_approved_exact_grant_for_owner("cli-test", &held, Some(4242))
            .unwrap()
    );
...
fn a_denied_or_stale_plan_neither_creates_nor_consumes_approvals() {
    ...
    assert!(
        crate::approvals::has_approved_exact_grant_for_owner("cli-test", &cap, Some(4242)).unwrap()
    );
