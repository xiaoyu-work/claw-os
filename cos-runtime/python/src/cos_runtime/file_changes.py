...
def replace_file(
    ...
    *,
    ...
    An explicit MCP call session selects the child transport's session only;
    the broker still authenticates it and the process environment is unchanged.
    """
    ...
    if session_id is not None:
        if not isinstance(session_id, str) or re.fullmatch(r"[A-Za-z0-9_.:-]{1,128}", session_id) is None:
            _invalid("replacement session must be a bounded canonical identifier")
        environment["env"] = {**os.environ, "COS_SESSION": session_id}
    payload = json.dumps(
