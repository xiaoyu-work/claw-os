...
def test_file_replace_uses_the_call_session_without_changing_global_environment(transport, monkeypatch):
    monkeypatch.setenv("COS_SESSION", "idle-session")
    monkeypatch.setenv("COS_EXTENSION_BROKER_SOCKET", "/run/test/broker.sock")
    file_changes.replace_file(PATH, state(), CONTENT, session_id="app-call-session")
    child = transport.call_args.kwargs["env"]
    assert child["COS_SESSION"] == "app-call-session"
    assert child["COS_EXTENSION_BROKER_SOCKET"] == "/run/test/broker.sock"
    assert os.environ["COS_SESSION"] == "idle-session"
    assert "app-call-session" not in repr(transport.call_args.args)
    assert "session" not in json.loads(transport.call_args.kwargs["input"])


@pytest.mark.parametrize("session_id", ["", "a" * 129, "line\nbreak", "nul\x00", "../other", "space id", 1])
def test_file_replace_rejects_invalid_call_sessions_before_transport(transport, session_id):
    with pytest.raises(file_changes.FileChangeError) as error:
        file_changes.replace_file(PATH, state(), CONTENT, session_id=session_id)
...
@pytest.mark.parametrize("call_session", [None, "transport-call-session"])
def test_file_replace_real_subprocess_uses_stdin_without_a_direct_write_fallback(tmp_path, monkeypatch, call_session):
    binary = tmp_path / "cos"
    binary.write_text(
        f"#!{sys.executable}\n"
        "import base64, hashlib, json, os, sys\n"
        "assert sys.argv[1:] == ['__file', 'replace']\n"
        f"assert os.environ.get('COS_SESSION') == {call_session or 'transport-test-session'!r}\n"
    ...
    binary.chmod(0o700)
    monkeypatch.setenv("CLAW_COS_BIN", str(binary))
    monkeypatch.setenv("COS_SESSION", "transport-test-session")
    target = tmp_path / "document"
    assert file_changes.replace_file(str(target), None, CONTENT, session_id=call_session) == result(path=str(target))
    assert os.environ["COS_SESSION"] == "transport-test-session"
