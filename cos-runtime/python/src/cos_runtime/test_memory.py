...
class SubprocessShellOutTests(unittest.TestCase):
    def test_remember_returns_envelope_from_stdout(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            cos = _write_fake_cos(tmp, stdout=json.dumps(envelope))
            with mock.patch.dict(os.environ, {"COS_BIN": cos}):
                got = memory.remember(text="hi", source="expense-tracker", indexable=False)
        self.assertEqual(got, envelope)

    def test_permission_denied_extracted_from_stderr(self) -> None:
        deny_envelope = {
            "decision": "deny",
            "verb": "memory.write",
            "summary": "memory.write not granted to source=calendar",
        }
        # The router wraps the deny in a "memory remember denied: {...}"
        # message on stderr with non-zero exit. The SDK should recover
        # the JSON and raise PermissionDenied.
        stderr = f"memory remember denied: {json.dumps(deny_envelope)}"
        with tempfile.TemporaryDirectory() as tmp:
            cos = _write_fake_cos(tmp, stderr=stderr, exit_code=1)
            with mock.patch.dict(os.environ, {"COS_BIN": cos}):
    ...
    def test_forget_by_source_invokes_correct_args(self) -> None:
        ...
        def fake_run(cmd, **kwargs):  # type: ignore[no-untyped-def]
            ...
            return _FakeProc(json.dumps(envelope), "", 0)

        with mock.patch.dict(os.environ, {"COS_BIN": "/usr/bin/cos"}):
