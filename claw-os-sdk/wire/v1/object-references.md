...
The resolver's existing `operation` field names an ordinary public App command;
its field name and the `id_arg` / `revision_arg` fields are unchanged. Under
the MCP-first manifest contract, `cos app <id> <command>` selects an explicit
`operations[command]` first, otherwise exactly the declared
`mcp.tools` entry named `<id>.<command>`. For example, an MCP-only Notes App can
bind `"operation": "get"` to `notes.get` without inventing a legacy `get`
operation or a Python `main.py` entrypoint. Selection is not an execution-error
fallback, and the removed `session` contract is not restored.

Before trusting a declaration, the backend must authenticate its App package
and validate the selected command's declaration. Bound ID/revision arguments
must exist and be non-repeatable string-kind arguments (`text`, `name`, `path`,
or `host`). The selected command must not consume opaque stdin or leave
required/conditional arguments unbound. Other arguments must not use
`trusted_resolver` or `default_from`.
...
App operation/MCP dispatch, capabilities, approvals, and audit. The SDK MCP
runtimes accept the `objects` metadata but advertise only `mcp.tools`; they
neither resolve objects nor execute an ordinary operation while loading it.
A resolver
...
[`manifest_extensions.vectors.json`](manifest_extensions.vectors.json) also
checks that all four MCP runtimes retain the MCP-first service while accepting
object declarations and optional MCP-tool or ordinary-operation effects.
Core owns the command-selector binding and verified resolver checks; these
SDK conformance cases do not claim that an App resolver was executed.
