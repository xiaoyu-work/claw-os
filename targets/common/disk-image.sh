#!/usr/bin/env bash
# targets/common/disk-image.sh — Shared persistent disk image packager.
#
# Output: build/<image-name>-<arch>.<fmt>  for each fmt in $FORMATS
...
#            Supported: qcow2, vmdk, vhdx, vhd, raw
#            vhd is fixed-size and intended for Azure; vhdx is for Hyper-V.
...
#   IMAGE_BASENAME
#            Output basename without architecture or extension.
#            Default: "claw-os-vm".
#   IMAGE_ROOTFS_FINALIZER
#            Optional script run against the staged filesystem after rsync
#            and before GRUB installation. Cloud targets use this to remove
#            per-instance identity without mutating the composed rootfs.
#
# Disk layout (GPT):
...
IMAGE_BASENAME="${IMAGE_BASENAME:-claw-os-vm}"
case "$IMAGE_BASENAME" in
    *[!A-Za-z0-9._-]*|"")
        echo "error: IMAGE_BASENAME must contain only letters, digits, '.', '_' or '-'" >&2
        exit 1
        ;;
esac
RAW="$BUILD_DIR/${IMAGE_BASENAME}-${ARCH_SUFFIX}.raw"
...
#
#    FEATURES is selected by the platform target. Example override for a
#    desktop VMware image:
#       FEATURES=base,cos-core,systemd,kernel,desktop,vmware,copilot-cli,grub-disk,vm,apt-source,local-user \
#       SIZE=16G FORMATS=vhdx ./build.sh vm
FEATURES="${FEATURES:?disk-image builder requires a target-selected FEATURES profile}"
...
# /etc/fstab — generated by targets/common/disk-image.sh
...
# Remove temporary host-side UUID links before platform finalization. The EXIT
# trap also removes them on failures, but the successful path disables that
# trap after unmounting.
for uuid_link in $UUID_LINKS; do
    rm -f "$uuid_link"
done
UUID_LINKS=""

# 11. Apply an optional platform finalizer as the last staged-filesystem
#     mutation. Nothing after this point may regenerate per-instance state.
if [ -n "${IMAGE_ROOTFS_FINALIZER:-}" ]; then
    if [ ! -f "$IMAGE_ROOTFS_FINALIZER" ]; then
        echo "error: IMAGE_ROOTFS_FINALIZER not found: $IMAGE_ROOTFS_FINALIZER" >&2
        exit 1
    fi
    echo ":: finalizing staged rootfs with $IMAGE_ROOTFS_FINALIZER"
    bash "$IMAGE_ROOTFS_FINALIZER" "$MNT"
fi
...
# 13. Convert raw -> requested formats. qemu-img convert preserves
#     filesystem UUIDs (block-level copy), so fstab UUID= entries stay
#     valid in every output. qcow2 is auto-sparse. Azure's VHD format is
#     emitted as fixed VPC with a 1 MiB-aligned virtual size.
for fmt in $FORMATS; do
    case "$fmt" in
        raw)
            ...
            ;;
        qcow2|vmdk|vhdx)
            OUT="$BUILD_DIR/${IMAGE_BASENAME}-${ARCH_SUFFIX}.$fmt"
            echo ":: qemu-img convert -f raw -O $fmt -> $OUT"
            qemu-img convert -f raw -O "$fmt" -S 65536 "$RAW" "$OUT"
            ;;
        vhd)
            OUT="$BUILD_DIR/${IMAGE_BASENAME}-${ARCH_SUFFIX}.vhd"
            echo ":: qemu-img convert -f raw -O vpc (fixed) -> $OUT"
            qemu-img convert -f raw -O vpc \
                -o subformat=fixed,force_size "$RAW" "$OUT"

            VHD_INFO="$(qemu-img info -f vpc --output=json "$OUT")"
            VHD_VIRTUAL_SIZE="$(
                printf '%s\n' "$VHD_INFO" \
                    | sed -n 's/^    "virtual-size":[[:space:]]*\([0-9][0-9]*\),*$/\1/p'
            )"
            if [ -z "$VHD_VIRTUAL_SIZE" ]; then
                echo "error: unable to read VHD virtual size from qemu-img" >&2
                exit 1
            fi
            if [ $((VHD_VIRTUAL_SIZE % (1024 * 1024))) -ne 0 ]; then
                echo "error: Azure VHD virtual size is not aligned to 1 MiB: $VHD_VIRTUAL_SIZE" >&2
                exit 1
            fi
            VHD_FILE_SIZE="$(stat -c '%s' "$OUT")"
            if [ "$VHD_FILE_SIZE" -ne $((VHD_VIRTUAL_SIZE + 512)) ]; then
                echo "error: VHD is not fixed-size (file=$VHD_FILE_SIZE virtual=$VHD_VIRTUAL_SIZE)" >&2
                exit 1
            fi
            ;;
        *)
...
echo
echo ":: done ($ARCH)"
ls -lh "$BUILD_DIR"/${IMAGE_BASENAME}-${ARCH_SUFFIX}.* 2>/dev/null

if [ "${IMAGE_PLATFORM:-vm}" = "azure" ]; then
    cat <<EOF

Azure generalized image:
  Artifact: build/${IMAGE_BASENAME}-${ARCH_SUFFIX}.vhd
  Hyper-V generation: V2
  Provisioning: cloud-init (Azure datasource) + walinuxagent

Upload and publish it with the commands in docs/building-azure.md.
EOF
