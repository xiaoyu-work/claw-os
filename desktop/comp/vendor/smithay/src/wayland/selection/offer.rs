    ...
    D: SelectionHandler + 'static,
{
    fn request(
        ...
        if !self.source.contains_mime_type(&mime_type) {
            ...
            let client = match dh.get_client(sender_id) {
                Ok(client) if client.id() == client_id => client,
                _ => {
                    debug!("Denying a {object_name}.receive without its requesting client");
                    return None;
                }
            };
            let Some(seat) = Seat::<D>::from_resource(&self.seat) else {
                debug!("Denying a {object_name}.receive without a seat");
                return None;
            };
            if !D::allow_selection_read(&client, &seat, self.target) {
                debug!(client = ?client.id(), target = ?self.target, "Denying a {object_name}.receive");
                return None;
            }
