...
COMMANDS = frozenset({"summary", "auth", "ssh", "sudo", "mac", "ports", "events"})
...
def _broker(action):
    cos_bin = _cos_binary()
    if not cos_bin:
        return {"error": "cos binary not found; Security Center broker unavailable"}
    try:
        ...
        return {"error": f"Security Center broker exceeded {TIMEOUT_SECS}s"}
    payload_text = (result.stdout or "").strip() or (result.stderr or "").strip()
    try:
        payload = json.loads(payload_text) if payload_text else {}
    except json.JSONDecodeError:
        return {"error": "Security Center broker returned invalid JSON"}
    if result.returncode != 0 and "error" not in payload:
        payload["error"] = f"Security Center broker exited {result.returncode}"
    return payload
