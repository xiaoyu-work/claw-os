...
An App can declare expected effects alongside an existing operation or MCP tool:
...
The command's runtime, arguments, capability needs, and execution behavior
remain unchanged. Effect declarations describe intent; they never grant
authority or prove what App code will actually do. Public command selection
prefers an explicit operation; otherwise the exact `<app-id>.<command>` MCP
tool supplies its own arguments and effects. MCP-only Apps need no legacy
operation or `main.py` entry.
...
App-declared effects, requested resource targets, unresolved optional arguments,
...
execution still resolves paths, performs consent
...
without `target_arg` is `unspecified`. Optional arguments with neither a
supplied value nor a literal default remain explicitly unresolved rather than
triggering credential lookup or a guessed provider choice. Retired runtime
selector fields are rejected by the manifest schema.
...
Each operation or MCP tool may declare at most 16 effects. Labels require English, are
