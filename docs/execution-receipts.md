...
Activity-associated tasks use the existing MCP App Mesh. Root captures each
service result from its retained prepared call, verified package identity and
task Activity association, before model-input trust fencing. One-shot CLI
operations are not projected as model tools. Schema inspection and
unassociated calls do not create automatic receipts.

Human `cos app` calls do not enable automatic task capture. The explicit
`cos operation execute --activity` path owns its single receipt even when the
human caller's session is already associated with that Activity.

The reporting-only adapter also accepts bounded task reports over the worker's
authenticated job channel, not through a new broker-socket permission.
The broker verifies the signed reporting route and
...
submitted it. The broker rechecks the stored task's owner and Activity before
writing that link, including for late reports after cancellation.
It records reporting provenance, not proof that the task
...
separate [extension and service Hosts](../core/src/extension_host/MODULE.md),
with root-owned invocation authorization, ordinary App permissions and
mandatory containment. A receipt never grants that execution authority.
...
operation. Its authenticated summary becomes the declaration label and its
own optional `effects` provide the declaration. Missing effects mean unknown
effects, not purity.

For session calls, the result hash and byte count name rendered tool output
before trust fencing, not the raw MCP envelope or omitted image bytes.
MCP `isError` produces `reported_error`; failures without a returned MCP
payload, including timeouts and lost/invalid responses, remain
`indeterminate`. Recording does not change Host failure/retirement policy.
Opening/closing a session and calls denied before dispatch do not create
execution receipts.
...
associated Agent MCP App calls through the shared root service manager.
