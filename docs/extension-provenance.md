...
Apps, Skills, MCP/adapter packages, and Agent extensions are third-party code and
...
schema is registered — the package is authenticated by a publisher signature
or an approved root-owned vendor package, and its **complete** file tree is
verified.
...
- `kinds` — which of `app` / `skill` / `mcp` / `extension` it may sign.
...
An out-of-process Agent extension uses package kind `extension`, manifest
`extension.json`, and only entrypoints declared in the authenticated envelope.
Activation is explicit through `agent.extensions`; discovery alone never runs
code. The worker verifies the installed package through the shared
`VerifiedPackage` API, and `claw-extension-host` independently re-verifies that
same installed identity and digest before materializing its task-private
snapshot. Neither process re-reads a mutable manifest after verification.

Developer grants do not activate Agent extensions. A user-installed extension
must be publisher-signed by a key whose `kinds` includes `extension`; a bundled
vendor extension must live under a root-owned approved system package root.
Verification, manifest, identity, version, digest, or entrypoint drift
quarantines the selected id with an actionable diagnostic rather than falling
