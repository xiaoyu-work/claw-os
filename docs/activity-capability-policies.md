...
They also do not replace protected owner App review or the App-service admission
gate. `cos review` and the native OS review presenter remain the review surfaces;
an Activity policy is not an App confirmation or a background-service grant.
...
Runtime confirmation checks use the shared whole-scope containment rules, so a
confirmation for a narrower symbolic pattern cannot authorize a broader request.
For runtime path checks, the **same allowed scope** must satisfy both whole-scope
containment and the ordinary symlink-aware `Scope::covers` check. This also
applies when the requested leaf does not exist yet; a lexical policy match alone
does not bypass filesystem authorization.

These additional path checks belong to runtime consumers. Pure policy
validation and matching still perform no filesystem I/O, and the catalogue and
policy forms do not resolve files or symlinks.
...
policy snapshot in the task stream. Private worker protocol 11 capability
mediation uses bounded verb/scope `Boundary`, `Consume` and `Request` messages,
not worker-supplied policy or identity selectors. This internal worker
contract does not change the public policy schema above. The recorded
snapshot supports reconstruction; it is not a model prompt enforcement scheme.

Main's isolated extension Host runs dynamic App/MCP code; the retired worker
AppHost control is not an execution fallback. Durable PREPARE/COMMIT admission
and nonce-, digest- and context-bound approvals remain in force. Activity policy
checks neither replace those bindings nor authorize execution before COMMIT.
