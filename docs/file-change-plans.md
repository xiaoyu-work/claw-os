...
The MCP command requires `fs.read` on the target and `data.db.write` on
...
Content is an explicit declared argument, supplied through `--content` or the
CLI's `--args-stdin` JSON argument object. Raw file content must not be piped
into the MCP transport. An empty proposal must be explicit (`--content ""`);
missing content is not interpreted as an empty replacement.
