...
- While idle with an empty composer, press `Esc` twice to select a retained
  user prompt. Selection creates a checked prefix fork and restores that prompt
  for editing; admitted effects are not rolled back.
...
- `/appearance` opens terminal-local theme, title and compact/full status-line
  controls. These settings do not mutate provider, task or persisted state.
- Appearance key `k` switches the composer between default bindings and
  bounded Vim NORMAL/INSERT modes. `/vim` is a direct compatible toggle;
...
Task model controls affect only subsequently submitted Jobs. Explicit
reasoning effort is persisted across queueing, retry and resume, and currently
...
for scenario in complete cancel commands confirmations durable-queue task-center task-controls approval-center attachments appearance file-mentions copy-export raw-scrollback vim backtrack notification-inbox activity-lifecycle activity-controls activity-evidence activity-review workspace multiline multiline-key reconnect startup-reconnect resume resume-running plain; do
...
The appearance scenario changes accent, terminal title and status density while
leaving backend state untouched.
The Vim scenario switches to NORMAL mode, enters INSERT, submits one prompt,
