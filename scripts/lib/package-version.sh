...
package_version() {
    ...
    if [ -z "$version" ]; then
        ...
        if ! git_readonly -C "$project_dir" rev-parse --git-dir >/dev/null 2>&1; then
            echo "error: package version requires a git checkout" >&2
            return 1
        fi
        if [ "$(git_readonly -C "$project_dir" rev-parse --is-shallow-repository)" = "true" ]; then
        ...
        if [ -n "$(git_readonly -C "$project_dir" status --porcelain --untracked-files=normal)" ]; then
        ...
        count="$(git_readonly -C "$project_dir" rev-list --count HEAD)"
        sha="$(git_readonly -C "$project_dir" rev-parse --short=12 HEAD)"
